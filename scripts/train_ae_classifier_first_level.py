...
import ignite
from ignite.engine import Engine, Events, create_supervised_trainer, create_supervised_evaluator, convert_tensor
...
def train(config, logger):
    ...
    model = config['model']
    ae_criterion = config['ae_criterion']
    ...
    config['metrics']['loss'] = Loss(cls_criterion)

    mlflow.log_param("model", get_object_name(model))
    mlflow.log_param("criterion", get_object_name(cls_criterion))
    mlflow.log_param("optimizer", get_object_name(optimizer))

    model.to(config['device'])

    def ae_update(engine, batch):
        model.train()
        optimizer.zero_grad()
        x = batch['image']
        x = convert_tensor(x, device=config['device'], non_blocking="cuda" in config['device'])
        x_pred, _ = model(x)
        loss = ae_criterion(x_pred, x)
        loss.backward()
        optimizer.step()
        return loss.item()

    ae_trainer = Engine(ae_update)

    def cls_update(engine, batch):
        model.train()
        optimizer.zero_grad()
        x, y = prepare_batch(batch, device=config['device'], non_blocking="cuda" in config['device'])
        _, y_pred = model(x)
        loss = cls_criterion(y_pred, y)
        loss.backward()
        optimizer.step()
        return loss.item()

    cls_trainer = Engine(cls_update)

    RunningAverage(output_transform=lambda x: x).attach(ae_trainer, 'ae_loss')
    ProgressBar().attach(ae_trainer, ['ae_loss'])

    @cls_trainer.on(Events.EPOCH_STARTED)
    def run_autoencoder(engine):
        logger.info("Start AE phase")
        ae_trainer.run(config['ae_loader'], max_epochs=3)
        logger.info("End AE phase")

    # add typical handlers
    setup_trainer_handlers(cls_trainer, logger, config)
    setup_log_learning_rate(cls_trainer, logger, config)
    ...
    setup_offline_train_metrics_computation(cls_trainer, logger, config)
    val_evaluator = setup_val_metrics_computation(cls_trainer, logger, config)
    setup_early_stopping(cls_trainer, config, val_evaluator, default_score_function)
    ...
    cls_trainer.run(train_dataloader, max_epochs=num_epochs)
